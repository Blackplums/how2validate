...
const contentPath = (
  ...
  return Settings.gitload
    ? `${GitHubLink.href}/raw/main/contents/${type}/${slug}/index.mdx`
    : path.join(process.cwd(), `/contents/${type}/`, `${slug}/index.mdx`)
}
...
const getContentPath = (() => {
  ...
  return (
    ...
    const key = `${type}-${slug}`
    if (!cache.has(key)) {
      cache.set(key, contentPath(type, slug))
    }
...
export async function getContent(
  ...
  try {
    ...
    const tocs = await getTable(type, slug)
...
export async function getTable(
  ...
  if (Settings.gitload) {
    const contentPath = `${GitHubLink.href}/raw/main/contents/${type}/${slug}/index.mdx`
    try {
      const response = await fetch(contentPath)
      if (!response.ok) {
        throw new Error(
