...
export default async function Pages({ params }: PageProps) {
  ...
  const res = await getContent("privacy-policy", pathName)
...
export function generateStaticParams() {
  return PageRoutes.filter((item) => {
    ...
    // Only check privacy-policy routes
    if (!item.href.startsWith("/privacy-policy/")) return false
    // Remove the leading "/privacy-policy/"
    const slug = item.href.replace(/^\/privacy-policy\//, "")
