...
export default async function Pages({ params }: PageProps) {
  ...
  const res = await getContent("acceptable-use-policy", pathName)
...
export function generateStaticParams() {
  return PageRoutes.filter((item) => {
    ...
    // Only check acceptable-use-policy routes
    if (!item.href.startsWith("/acceptable-use-policy/")) return false
    // Remove the leading "/acceptable-use-policy/"
    const slug = item.href.replace(/^\/acceptable-use-policy\//, "")
    // Check if the file exists
    const filePath = path.join(
      process.cwd(),
      "contents/acceptable-use-policy",
      slug,
      "index.mdx"
    )
