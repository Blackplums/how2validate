...
export default async function Pages({ params }: PageProps) {
  ...
  const res = await getContent("blog", pathName)
...
export function generateStaticParams() {
  return PageRoutes.filter((item) => {
    ...
    // Only check blog routes
    if (!item.href.startsWith("/blog/")) return false
    // Remove the leading "/blog/"
    ...
    slug: item.href.split("/").slice(2), // slice(2) to remove ['', 'blog']
  }))
}
