...
export default async function Pages({ params }: PageProps) {
  ...
  const res = await getContent("third-party-service-disclosure", pathName)
...
export function generateStaticParams() {
  return PageRoutes.filter((item) => {
    ...
    // Only check third-party-service-disclosure routes
    if (!item.href.startsWith("/third-party-service-disclosure/")) return false
    // Remove the leading "/third-party-service-disclosure/"
    const slug = item.href.replace(/^\/third-party-service-disclosure\//, "")
    // Check if the file exists
    const filePath = path.join(
      process.cwd(),
      "contents/third-party-service-disclosure",
      slug,
      "index.mdx"
    )
