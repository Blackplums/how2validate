...
export default async function Pages({ params }: PageProps) {
  ...
  const res = await getContent("terms-conditions", pathName)
...
export function generateStaticParams() {
  return PageRoutes.filter((item) => {
    ...
    // Only check terms-conditions routes
    if (!item.href.startsWith("/terms-conditions/")) return false
    // Remove the leading "/terms-conditions/"
    const slug = item.href.replace(/^\/terms-conditions\//, "")
